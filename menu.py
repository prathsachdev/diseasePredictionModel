...
mydb = None
tables = []
dummyTables = ["dummyTable1", "dummyTable2"]

def createDummyTables():
    global mycursor, mydb
    try:
        for i in range(len(dummyTables)):
            mycursor.execute(f"create table {dummyTables[i]}(No varchar(10), Name varchar(255), Addr varchar(255));")
            for j in range(2):
                mycursor.execute(f"insert into {dummyTables[i]} values('val{j}', 'val{j+1}', 'val{j+2}')")
...
def setcursor(cursor, db):
    global mycursor, mydb, tables
    mycursor = cursor
    mydb = db

    try:
        mycursor.execute("create database pythonDatabase;")
        mycursor.execute("use pythonDatabase")
        sqlcode = "show tables"
        createDummyTables()
...
def newtab():
    ...
    def showtable():
        ...
        temp = end
        for i in range(0, clen):
            columns[i] = (Entry(tabwn, width=25, bg="white"))
            ...
            for j in range(start, end):
                rows[j] = (Entry(tabwn, width=20, bg="white"))
                rows[j].grid(row=toplace + 3, column=i)
                toplace += 1
                if j == end - 1:
                    toplace = 0
        tablename = (Entry(tabwn, width=20, bg='white'))
        ...
        def tabcreation():
            ...
            def savetab():
                ...
                Label(msg, text=tabname + " successfully saved to the database ", bg=themebg, fg=themefg,
                      font="normal 12 bold").grid(row=0, column=0)
                tabwn.destroy()
            ...
            if tabfound == 0 and createtab == 0:
                for i in range(0, clen):
                    if i == 0:
                        sqlcode = "create table if not exists " + tabname + " " + "(" + columnnames[
                            0] + " " + columntype + "(" + columnlen + ")" + ")"
                        mycursor.execute(sqlcode)
                    else:
                        sqlcode = "alter table " + tabname + " add " + columnnames[
                            i] + " " + columntype + " (" + columnlen + ");"
                        mycursor.execute(sqlcode)
                for j in range(0, rlen * clen):
                    compare += 1
                    if j < rlen:
                        sqlcode = ("insert into " + tabname + "(" + columnnames[toplace] + ") values (' " + rownames[
                            j] + " ' )")
                        mycursor.execute(sqlcode)
                    else:
                        if toplace > 1:
                            sqlcode = ("update " + tabname + " set " + columnnames[toplace] + " = ' " + rownames[
                                j] + " ' where " + columnnames[toplace - 1] + " = ' " + rownames[
                                           j - rlen] + " '  and " + columnnames[toplace - 2] + " = ' " + rownames[
                                           j - rlen * 2] + " ' ")
                            mycursor.execute(sqlcode)
                        else:
                            sqlcode = ("update " + tabname + " set " + columnnames[toplace] + " = ' " + rownames[
                                j] + " ' where " + columnnames[toplace - 1] + " = ' " + rownames[j - rlen] + " ' ")
                            mycursor.execute(sqlcode)
                    if compare == rlen:
                        compare = 0
                        toplace += 1
...
def selecttab(tabindex, buttons, tables):
    global selectedindex, selectedtable
    if selectedindex is not None:
        buttons[selectedindex].config(bg=themebg, fg=themefg)
    buttons[tabindex].config(bg="yellow", fg="black")
    selectedindex = tabindex
    selectedtable = tables[tabindex]
...
def viewndeltab():
    ...
    for i in mycursor:
        tables.append(i)
    for i in range(0, len(tables)):
        button = Button(wn, text=tables[i], bg=themebg, fg=themefg, width=20, height=1,
                        command=lambda i=i: selecttab(i, buttons, tables))
    ...
    def deltab():
        if checkselected() == 1:
            ...
            if tabfound == 1:
                Label(wn, text="Are you sure you want to delete the above table", bg=themebg, fg=themefg).grid(
                    row=i + 5, column=0, columnspan=2, sticky=W)
                ...
                Label(wn, text="This table doesnot exist", bg=themebg, fg=themefg).grid(row=2, column=0, columnspan=2,
                                                                                        sticky=W)
    ...
    def view():
        if checkselected() == 1:
            ...
            if tabfound == 1:
                ...
                Label(wn, text="The tablename you searched for doesnot exist ", bg=themebg, fg=themefg,
                      font="normal 11 bold").grid(row=i + 3, column=0, columnspan=3)
...
def insertdata():
    ...
    for i in mycursor:
        tables.append(i)
    for i in range(0, len(tables)):
        output = Button(wn, text=tables[i], width=20, height=1, bg=themebg, fg=themefg,
                        command=lambda i=i: selecttab(i, outputs, tables))
        ...
        if tabfound == 1:
            ...
            def insertrow():
                ...
                for k in range(0, 4):
                    ...
                    editcolumns[k] = (Entry(wnn, width=20, bg="white"))
                    Label(wnn, text=textt[k], bg=themebg, fg=themefg, font="normal 11 bold").grid(row=len(data) + 1,
                                                                                                  column=k)
            ...
            def update():
                ...
                for c in range(0, len(editcolumns)):
                    ...
                    val.strip()
                    if val == '':
                        Label(wnn, text="Insufficient data", bg=themebg, fg=themefg, font="normal 11 bold").grid(row=20,
                                                                                                                 column=0,
                                                                                                                 columnspan=3)
                        break
                    else:
                ...
                if updatedata == 1:
                    try:
                        sqlcode = "update " + toedit + " set " + ndata[0] + " = ' " + ndata[1] + " ' where " + ndata[
                            2] + " = ' " + ndata[3] + " ' ; "
                        mycursor.execute(sqlcode)
                        mydb.commit()

                        Label(wnn, text="The data has been updated succesfully ", fg=themefg, bg=themebg).grid(
                            row=len(data) * 2, column=0)
                        incre += 1

                    except:
                        Label(wnn, text="Reference data doesnot exist", bg=themebg, fg=themefg,
                              font="normal 11 bold").grid(row=20, column=0, columnspan=3)
                else:
                    Label(wnn, text="Reference data doesnot exist", bg=themebg, fg=themefg, font="normal 11 bold").grid(
                        row=20, column=0, columnspan=3)

            Button(wnn, text="update", command=update).grid(row=len(data) * 3, column=int(len(toshowcolumns) / 2))


        else:
            Label(wn, text="The tablename you searched for doesnot exist ", bg=themebg, fg=themefg,
                  font="normal 11 bold").grid(row=4, column=0, columnspan=3)
            ...
            def insertsave():
                ...
                for e in range(1, len(toshowcolumns)):
                    if ndata[e] == '':
                        ndata[e] = "NULL"
                    if e == 1:
                        sqlcode = "update " + toedit + " set " + toshowcolumns[e] + " = ' " + ndata[e] + " '  where " + \
                                  toshowcolumns[0] + " = ' " + ndata[0] + " ' ; "
                    else:
                        sqlcode = "update " + toedit + " set " + toshowcolumns[e] + " = ' " + ndata[e] + " ' where  " + \
                                  toshowcolumns[0] + " = ' " + ndata[0] + " ' and " + toshowcolumns[1] + " = ' " + \
                                  ndata[1] + " ' ;"
                    mycursor.execute(sqlcode)
                mydb.commit()
                Label(wnn, text="The data has been inserted succesfully ", fg=themefg, bg=themebg).grid(row=17,
                                                                                                        column=0)
                incre += 1
                Button(wnn, text="add another ", bg=themebg, fg=themefg, command=insertrow).grid(row=16, column=int(
                    len(toshowcolumns) / 2))
...
def deletedata():
    ...
    for i in mycursor:
        tables.append(i)
    for i in range(0, len(tables)):
        output = Button(wn, text=tables[i], width=20, bg=themebg, fg=themefg, height=1,
                        command=lambda i=i: selecttab(i, outputs, tables))
        output.grid(row=i, column=0, columnspan=2)
        outputs.append(output)
    ...
    def opentodeltab():
        ...
        if tabfound == 1:
            ...
            for i in range(0, len(fdata)):
                outputs[i].delete(0.0, END)
                outputs[i].insert(END, fdata[i])
            Label(wnn, text="Enter the %s from the table to delete" % toshowcolumns[0], bg=themebg, fg=themefg,
                  font="normal 11 bold").grid(row=i, column=0)
            ...
            def delrow():
                ...
                if datafound == 1:
                    ...
                    mycursor.execute(sqlcode)
                    mydb.commit()
                    Label(wnn, text="The data had been deleted sucessfully ", bg=themebg, fg=themefg,
                          font="normal 11 bold").grid(row=i + 15, column=0)
                    wnn.destroy()
                    wnnn = Tk()
                    wnnn.title("Operation sucessful ")
                    wnnn.config(background=themebg)

                    Label(wnnn, text="The data has been sucessfully deleted", bg=themebg, fg=themefg,
                          font="normal 11 bold").grid(row=0, column=0)

                else:
                    print(len(data))
                    Label(wnn, text="The data does not exist ", bg=themebg, fg=themefg, font="normal 11 bold").grid(
                        row=len(data) + 15, column=0)
    ...
    Button(wn, text="open table", command=opentodeltab).grid(row=i + 4, column=0, columnspan=3)
...
def modify():
    ...
    def modifyy():
        ...
        if rawcustomdata[0] in colours and rawcustomdata[1] in colours:
            ...
            Label(wn, text="The colour you entered isnt available", bg=themebg, fg=themefg, font="normal 11 bold").grid(
                row=6, column=0)
    ...
    Label(wn, text="Enter the background colour you want for the window ", bg=themebg, fg=themefg,
          font="normal 11 bold").grid(row=0, column=0)
    Label(wn, bg=themebg).grid(row=1, column=0)
    Label(wn, text="Enter the text colour you want for the window ", bg=themebg, fg=themefg,
          font="normal 11 bold").grid(row=2, column=0)
    ...
    Button(wn, text="Customise", command=modifyy, bg=themebg, fg=themefg, font="normal 11 bold").grid(row=4, column=0)
    Button(wn, text="Restore to default", command=default, bg=themebg, fg=themefg, font="normal 11 bold").grid(row=5,
                                                                                                               column=0)
...
def start():
    ...
    Select from below which one you would like to do.""", bg=themebg, fg=themefg, font="none 15 bold").grid(
        columnspan=3)

    Label(screen, bg=themebg, width=20).grid(row=1, column=0)
    Button(screen, text="Create a new table ", fg=themefg, bg=themebg, font="normal 11 bold", command=newtab,
           width=15).grid(row=2, column=0, columnspan=3)

    Label(screen, bg=themebg, width=20).grid(row=3, column=0)
    Button(screen, text="View/Delete table", fg=themefg, bg=themebg, font="normal 11 bold", command=viewndeltab,
           width=15).grid(row=4, column=0, columnspan=3)

    Label(screen, bg=themebg, width=20).grid(row=5, column=0)
    Button(screen, text="Insert / Update data", fg=themefg, bg=themebg, font="normal 11 bold", command=insertdata,
           width=15).grid(row=6, column=0, columnspan=3)

    Label(screen, bg=themebg, width=20).grid(row=7, column=0)
    Button(screen, text="Delete data", fg=themefg, bg=themebg, font="normal 11 bold", command=deletedata,
           width=15).grid(row=8, column=0, columnspan=3)
    mainloop()
